...
void gen_expression(A_NODE *node) {
    ...
    switch (node->name) {
        ...
        case N_EXP_FLOAT_CONST :
            i = node->clink;
            gen_code_f(LOD, 0, literal_table[i].addr);
            break;
    ...
    switch (node->name) {
        ...
        case N_STMT_CONTINUE :
            if (cnt_label) {
                ...
                gen_error(22, node->line, NULL);  // 발생 불가능한 에러
            }
            break;
        case N_STMT_BREAK :
            if (brk_label) {
                ...
                gen_error(23, node->line, NULL);  // 발생 불가능한 에러
            }
            break;
        case N_STMT_RETURN :
            n = node->clink;
...
void gen_declaration(A_ID *id) {
    ...
    A_NODE *node;
    switch (id->kind) {
        /** 선언문은 본 과제에서 하지 않기로 약속했다 */
        case ID_VAR:
            /** 초기화는 강의에서 아예 다루지 않는다 약속했다 */
            if (id->init) {
                if (id->level == 0) {
                    // gen_initializer_global();
                }
                else {
                    // gen_initializer_local();
                }
            }
            break;
        case ID_FUNC:
            /** prototype은 하지 않고, body(type table->expr)가 존재할 경우만 한다 */
            if (id->type->expr) {
