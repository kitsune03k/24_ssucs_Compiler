...
%token LP RP LB RB LR RR PERIOD COMMA EXCL STAR SLASH PERCENT ASSIGN COLON
%token AUTO_SYM STATIC_SYM TYPEDEF_SYM STRUCT_SYM ENUM_SYM SIZEOF_SYM UNION_SYM
%token IF_SYM ELSE_SYM WHILE_SYM DO_SYM FOR_SYM CONTINUE_SYM BREAK_SYM
...
function_definition
    ...
    | declarator {$$ = setFunctionDeclaratorSpecifier($1, makeSpecifier(int_type, NIL));}
...
declaration_specifiers
    : type_specifier {$$ = makeSpecifier($1, NIL);}
    | storage_class_specifier {$$ = makeSpecifier(NIL, $1);}
    | type_specifier declaration_specifiers {$$ = updateSpecifier($2, $1, NIL);}
    | storage_class_specifier declaration_specifiers {$$ = updateSpecifier($2, NIL, $1);}
    ;
...
direct_declarator
    ...
    parameter_type_list_opt RP {checkForwardReference(); --current_level; current_id = $3; $$ = setDeclaratorElementType($1, setTypeField(makeType(T_FUNC), $4));}
    ;
...
statement_list_opt
    : {$$ = makeNode(N_STMT_LIST_NIL, NIL, NIL, NIL);} // no option
    | statement_list {$$ = $1;}
    ;
...
compound_statement
    : LR {$$ = current_id; ++current_level;}
    declaration_list_opt statement_list_opt RR {checkForwardReference(); $$ = makeNode(N_STMT_COMPOUND, $3, NIL, $4); --current_level; current_id = $2;}
    ;
...
arg_expression_list_opt
    : {$$ = makeNode(N_ARG_LIST_NIL, NIL, NIL, NIL);} // no option
